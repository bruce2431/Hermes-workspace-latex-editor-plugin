...
export const FileTree: React.FC<FileTreeProps> = ({ files, activeFileId, onSelectFile, onClose }) => {
  return (
    <aside className="w-full bg-ide-surface flex flex-col h-full shrink-0 font-sans">
      <div className="h-[40px] bg-ide-panel border-b border-ide-border flex items-center justify-between px-4 text-[12px] font-medium text-ide-text shrink-0">
        <span>Explorer</span>
        {onClose && (
          <button onClick={onClose} className="text-ide-muted hover:text-ide-text transition-colors" title="Close Explorer">
      ...
      <div className="py-3 flex-1 overflow-y-auto">
        <div className="py-1.5 px-4 text-[11px] font-semibold tracking-wider flex items-center gap-2 text-ide-muted mb-1">
